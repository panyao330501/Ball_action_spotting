...
12. 所有使用 GPU 的任务，包括模型推理、训练、冒烟测试和 GPU 诊断，均必须通过 Slurm 提交；禁止绕过 Slurm 在登录节点或普通交互式 Shell 中直接运行 GPU 工作负载。
13. GitHub 仅同步代码、配置和文档。输入 MP4、权重、抽帧、预测或渲染视频不得通过 Git 传输。
14. 不允许本地和远端独立修改同一个源文件。应在修改端提交并推送，再在另一端拉取。
15. 阈值处理前必须保留原始模型分数，以便调整可视化阈值时无需重新执行推理。
16. `OFFSCREEN` 区间属于输入不可观测，不自动视为模型漏检。除非确实加入空间模型，否则不得暗示空间定位能力或绘制检测框。
17. 保留原始视频不变。派生媒体必须写入被 Git 忽略的产物或输出目录。
18. 工作文档正文使用中文；命令、路径、代码标识、标准标签和字段名保留其原始精确拼写。
